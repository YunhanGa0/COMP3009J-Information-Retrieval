...
-  punctuation is generally removed  (去标点符号)
...
- Document #1's CombSUM score is 0.45 + 0.3 + 0.35 = 1.

**Score Normalisation**

**目的：**解决不同系统**评分范围不同**的问题

- 比如系统A是0-1000
- 系统B是0-1

$$
\text{normalised\_score} =   \frac{normalised\_score-min\_score}{max\_score-min\_score}
$$

### CombMNZ (Multiply by Non Zero)

**CombMNZ** = 文档的**CombSUM***非零的系统数
...
-  Many backlinks (被很多文档链接)
-  Backlinks with high PageRank (被PageRank高分文档链接)

$$
R(u) = \sum_{v \in B_u} \frac{R(v)}{N_v}
$$

- $$B_u$$：指向节点 $$u$$ 的所有邻居节点集合（Backlinks）。
- $$N_v$$：节点 $$v$$的出链数量（Out-degree）。
- $$\frac{R(v)}{N_v}$$：节点 $$v$$ 的排名均匀分配给它的所有出链。

| 文档A有100PageRank分数和2个出链：<br>A给C和D各发送了50分<br />文档B有9PageRank分数和3个出链：<br />给三个出链对象各3分 | ![](.\屏幕截图 2025-05-19 211714.png) |
| ------------------------------------------------------------ | ------------------------------------- |

### Question:

如果我们需要 PageRank 分数来计算 PageRank，那么**初始 PageRank** 从何而来？

- 在初始阶段可以赋予任意分数
- 通过公式计算新结果，直到**分数收敛**（即再次计算不会改变分数，或者改变分数很小）

### Problems

- PageRank在应对**rank sink**情况时表现非常不好
- 在rank sink中，PageRank 在组中累积，从而导致**人为地提高** PageRank 分数

![](C:\Users\gaoyunhan\OneDrive\大三下\信息检索 IR\COMP3009J-Information-Retrieval\笔记\屏幕截图 2025-05-19 212828.png)

### 应对Rank Sink的新PageRank公式

$$
R(u) = (1-d)+d*\sum_{v \in B_u} \frac{R(v)}{N_v}
$$

- 这个公式增加了一个**damping factor**(d)，它确保不是文档的所有 PageRank 都是通过其外链传递的

# 11a-Relevance Feedback

**Relevance Feedback**是指当用户参与到检索过程中，以改进最终结果集。

### Rocchio Algorithm

把所有文档和查询关键词都用**向量**代表，找到一个查询向量，使其与**相关**文档相似性最**高**，并于**不相关**文档相似性最**低**
$$
\vec{q}_{opt} = \arg \max_{\vec{q}} \left[ \textit{sim}(\vec{q}, C_r) - \textit{sim}(\vec{q}, C_{nr}) \right]
$$
...
$$
\vec{q}_m = \alpha \vec{q}_0 + \beta \biggl( \frac{1}{|D_r|} \sum_{\vec{d}_j \in D_r} \vec{d}_j \biggr) - \gamma \biggl( \frac{1}{|D_{nr}|} \sum_{\vec{d}_j \in D_{nr}} \vec{d}_j \biggr)
$$
...
Assume the top k ranked documents are relevant and then apply a relevance feedback algorithm

###  Implicit Relevance Feedback 

Evidence from other sources:  clicks and user behaviour

# 11b-Query Expansion

选择术语并将其添加到用户的**Query**中，目的是最大限度地减少查询文档不匹配，从而提高检索性能
...
- **Synonymy**: Query的同义词
  - **手动**thesaurus: 人工维护
  - **自动派生**thesauruss
  - 使用**单词嵌入(word embeddings)**作为类似效果，而不使用同义词库。

> Query Expansion is the process of adding terms to a query in order to **increase recall**.

# 12a-Web Crawling

**Web Crawlers** are programs that automatically look for  documents on the web that will be included in the index so that users can find  them when searching.

> Web 爬虫或 Web 蜘蛛是自动在 Web 上查找将包含在索引中的文档的程序，以便用户在搜索时可以找到它们



# 12b-Adversarial Informatiosn Retrieval
